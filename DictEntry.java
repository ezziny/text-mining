...
// mourad_edit1
// add posting function used by doBiword function 
   void addPosting_Biword(int i) {
       // pList = new Posting(i);
       if (pList == null) {
           pList = new Posting(i);
           last = pList;
...
// mourad_edit1
// add posting function used by doPositionalIndex function 
void addPosting_PositionalIndex(int docId, int position) {
    if (pList == null) {
        pList = new Posting(docId, position);
        last = pList;
    } else {
        last.next = new Posting(docId, position);
        last = last.next;
    }
}
