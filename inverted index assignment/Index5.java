    ...
    for (String fileName : files) {
        try (BufferedReader file = new BufferedReader(new FileReader(fileName))) {
            ...
            while ((ln = file.readLine()) != null) {
                ...
                flen += indexOneLine(ln, fid); // Call indexOneLine method to process each line
            }
    ...
    public int indexOneLine(String ln, int fid) {
        ...
        for (String word : words) {
            ...
            if (!index.get(word).postingListContains(fid)) {
                index.get(word).addPosting(fid, position); // Add position along with document ID
            } else {
...
//    public String findDocument(String phrase) {
//        // Check if the phrase contains an underscored word combination
//        if (phrase.contains("_")) {
//            // Handle underscored word combination query
//            return handleUnderscoreQuery(phrase);
//        } else {
//            // Handle regular single-word query
//            return handleRegularQuery(phrase);
//        }
//    }
//    private String handleUnderscoreQuery(String phrase) {
...
//            String underscoredWord = word + "_" + nextWord;
//
//            // Search for the underscored word combination in the inverted index
//            if (index.containsKey(underscoredWord)) {
//                // Get the posting list for the underscored word combination
//                Posting posting = index.get(underscoredWord).pList;
//
//                // Append the documents containing the underscored word combination to the result
//                while (posting != null) {
...
//                }
//            } else {
//                // Handle the case where the underscored word combination is not found in the index
//                System.out.println("Underscored word combination '" + underscoredWord + "' not found in the index.");
...
//    }
//    private String handleRegularQuery(String phrase) {
//        StringBuilder result = new StringBuilder();
//
//        // Split the phrase into individual words
//        String[] words = phrase.split("\\s+");
...
//
//                // Append the documents containing the word to the result
//                while (posting != null) {
//                    result.append("\t").append(posting.docId).append(" - ").append(sources.get(posting.docId).title).append(" - ").append(sources.get(posting.docId).length).append("\n");
//                    posting = posting.next;
//                }
